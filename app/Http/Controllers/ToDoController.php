...
class ToDoController extends Controller
{
    public function index(){
        ...
        $friends = User::where('is_admin',1)->get();
        }
        return view("index",compact("tasks","friends","invitations"));
    }    
    ...
    public function sendInvitation(Request $request){
    	if ((int) $request->input('admin') > 0 
            && !Invitation::where('friend_id',Auth::user()->id)->where('admin_id',$request->input('admin'))->exists()
            ) {
            $invitation = new Invitation;
            $invitation->friend_id = Auth::user()->id;
